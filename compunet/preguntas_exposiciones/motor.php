...
function iniciar($soluciones, $mc = [], $texto = []) {
    ...
    foreach ($MC as $id => $p) {
        /* Una pregunta con 'multiple' => true se marca con casillas y solo
           cuenta como correcta si el conjunto marcado es EXACTAMENTE el
           esperado: ni de menos ni de mas. */
        $esMulti = !empty($p['multiple']);

        if ($VER_SOL) {
            $MCRESP[$id]  = $esMulti ? $p['correctas'] : $p['correcta'];
            $MCVERIF[$id] = 'correcto';
            continue;
        }

        if ($esMulti) {
            $r = (isset($_POST["mc_$id"]) && is_array($_POST["mc_$id"]))
                 ? array_map('strval', $_POST["mc_$id"]) : [];
            $MCRESP[$id] = $r;
            if (!$r) { $MCVERIF[$id] = ''; continue; }
            $marcadas = $r;              sort($marcadas);
            $buenas   = array_map('strval', $p['correctas']); sort($buenas);
            $MCVERIF[$id] = ($marcadas === $buenas) ? 'correcto' : 'incorrecto';
            continue;
        }
...
function mc($id) {
    ...
    echo '<p class="enunciado">' . $p['texto'] . ' ' . _marca($c);
    if ($esMulti) echo ' <span class="varias">varias correctas</span>';
    echo '</p>';

    /* En las de varias correctas no se rota el orden: la rotacion esta
       pensada para que la respuesta unica no caiga siempre en la misma letra. */
    $orden = $esMulti ? array_keys($p['opciones']) : _orden_opciones($p);
    $marcadas = $esMulti && is_array($r) ? $r : [];
    $i     = 0;
    foreach ($orden as $clave) {
        $texto = $p['opciones'][$clave];
        if ($esMulti) {
            $checked = in_array((string) $clave, $marcadas, true) ? ' checked' : '';
            $marca   = ($c !== '' && in_array((string) $clave, array_map('strval', $p['correctas']), true))
                       ? ' es-correcta' : '';
...
.opcion input{margin-right:4px;transform:scale(1.15);vertical-align:middle}
.varias{display:inline-block;background:#e8eef8;color:var(--azul);border-radius:20px;
        padding:1px 10px;font-size:12px;font-weight:700;vertical-align:middle}
...
@media(max-width:700px){
  ...
  /* Cinco botones no caben en 390px: se dejan en dos filas en vez de
     salirse por los lados. */
  .pie{flex-wrap:wrap;justify-content:center;gap:8px;padding:10px 10px}
  .pie .btn-ok,.pie .btn-gris{padding:8px 12px;font-size:14px;flex:0 0 auto}
  body{padding-bottom:130px}
  .enunciado{font-size:15.5px}
  .opcion{font-size:14.5px;padding:6px 6px}
}
</style>
<script>
